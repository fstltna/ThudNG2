...
public class MUStatusComponent extends JScrollPane {
	...
	private SimpleAttributeSet conRegular, conIrregular, conCriticals;
	...
	private void initAttributeSets () {
		...
		conCriticals = new SimpleAttributeSet ();
		StyleConstants.setFontFamily(conCriticals, prefs.mainFont);
		StyleConstants.setFontSize(conCriticals, prefs.statusFontSize);
		StyleConstants.setForeground(conCriticals, Color.white);
		StyleConstants.setBold(conCriticals, true);
		StyleConstants.setBackground(conCriticals, MUColors.r);
	}
	...
	public void refresh (final MUData data) {
		...
		if (mydata.sCriticals == null)
			addBlankLine();
		else
		{
			addString("CRITICALS: " + mydata.sCriticals + "\n", conCriticals);
			addString("\n",conRegular);
			addBlankLine();
		}
