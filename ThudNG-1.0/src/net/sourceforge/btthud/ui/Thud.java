...
import net.sourceforge.btthud.ui.configuration.*;
...
public class Thud extends JFrame implements Runnable {
    ...
    boolean					bReplay = false;
    
    MUConnectionBase 		conn = null;
    MUParse					parse = null;
    ...
    MUTacticalMap			tacMap2 = null;
    MUPrefs					prefs = null;
    MUCommands				commands = null;
    
    public	MUMapActions	mapActions = null;
    
    protected Thud			mySelf;
	...
	private ThudAction taResetDefaults;
	private ThudAction taHotKeys;
	...
	private ThudAction taStopMapKeysAndClicks;
	...
	private ThudAction taPauseResumeLog;
	private ThudAction taSaveReplay;
	private ThudAction taShowReplay;
	private ThudAction taFastForward;
	private ThudAction taFastFastForward;
	private ThudAction taPauseReplay;
	private ThudAction taContinueReplay;
	private ThudAction taMarkLocation;
	private ThudAction taSaveReplayFromMark;
	...
	{
		String sVersion = new Integer (iVersion).toString () + "." +
			new Integer (iSubVersion).toString () + "." +
			new Integer (iNanoVersion).toString ();
		
		return sVersion;
	}
	
	public MUTacticalMap getTacmap ()
	{
		return tacMap;
	}
	...
	private Thud (String[] arguments) {		
		...
		setTitle ("ThudNG2 " + getVersion ());
		
		myHotKeys = null;
		
		mySelf = this;
		
		myLogger = new Logger ("ThudNG");
		...
		textPaneWriter.println(" *** ThudNG, (c) 2009 Julian Brown & the THUDNG/THUD team    ***");
		textPaneWriter.println(" *** ThudNG2, (c) 2016 Marisa Giancarla                      ***");

		textPaneWriter.println(" *** Version: " + getVersion ());
		textPaneWriter.println();
		textPaneWriter.println(" *** To get started, connect to a MUX via the HUD menu,      ***");
		textPaneWriter.println(" *** then hit Ctrl-G when in a combat unit to activate ThudNG2! ***");
	...
	private void addHUDMenu () {
		...
		hudMenu.add(taResetDefaults);
		
		hudMenu.addSeparator();

		hudMenu.add(taHotKeys);
	...
	private void addMapMenu () {
		...
		addCheckBoxItem(mapMenu, taStopMapKeysAndClicks);
	...
	private void addReplayMenu () {
		...
		replayMenu = new JMenu ("Log/Replay");

		replayMenu.add(taPauseResumeLog);
		replayMenu.add(taSaveReplay);
		replayMenu.add(taShowReplay);

		replayMenu.addSeparator();
		
		replayMenu.add(taMarkLocation);
		replayMenu.add(taSaveReplayFromMark);
		
		replayMenu.add(taPauseReplay);
		replayMenu.add(taContinueReplay);
		replayMenu.add(taFastForward);
		replayMenu.add(taFastFastForward);
		
		// Disable the map menu until we're actually connected
		replayMenu.setEnabled(true);
		mainMenuBar.add(replayMenu);
	}

	private void addResourcesMenu () {
...
//		bindCommand(KeyEvent.VK_NUMPAD9, Event.ALT_MASK, "rottorso r");
		...
		instantiateMapActions ();
		
		int 	iLen;
		int		iIdx;
		MUMapAction myAction;
		
		myLogger.Log("ThudWindow: actions 001");
		
		iLen = mapActions.numActions ();
		for (iIdx = 0; iIdx < iLen; ++iIdx)
		{
			myAction = mapActions.getAction(iIdx);
			myLogger.Log("ThudWindow: actions 002 checking :" + myAction.forConfigList() + ":");
			myLogger.Log("ThudWindow: actions 002.01 checking :" + new Boolean (myAction.CanUseInChat ()).toString () + ":");

			if (myAction.CanUseInChat ())
			{
				myLogger.Log("ThudWindow: actions 003 adding :" + myAction.forConfigList() + ":");
				
				bindMUMapAction (myAction);
			}
		}
		...
		taHotKeys = new ThudSimpleAction ("Configure Hot Keys") {
			protected void doAction () {
				if (myHotKeys == null)
				{
					myHotKeys = new HotKeys ();
				}
		...
		String sLog = "Pause Logging";
		if (Logger.getNoLog ())
			sLog = "Resume Logging";
		
		taPauseResumeLog = new ThudSimpleAction ("Pause Log") {
			protected void doAction () {
				if (Logger.getNoLog ())
				{
					Logger.setNoLog(false);
					this.putValue (Action.NAME, "Pause Logging");
				}
				else
				{
					Logger.setNoLog(true);
					this.putValue (Action.NAME, "Resume Logging");
				}
			}
		};
		
		taSaveReplay = new ThudSimpleAction ("Convert Log to Replay") {
			protected void doAction () {
				String myHomeDir = System.getProperty("user.home");
				String sLogFile = myHomeDir + "/ThudLog.txt";
				
				String sOutputName = myHomeDir + "/ThudReplay.txt";
				...
				ParseThudLogForReplay.parseLogToReplay(sLogFile, fc.getSelectedFile ().getAbsolutePath ());
			}
		};
		...
		taSaveReplayFromMark = new ThudSimpleAction ("Save Replay From Mark to Here") {
			protected void doAction () {
				if (!bReplay)
					return;
				...
				int iStart;
				int iEnd;
				MUReplayConnection myReplay;
				
				myReplay = (MUReplayConnection)conn;
				
				iStart = myReplay.getMarkedLine();
				iEnd = myReplay.getCurrentLine ();
				
				ParseThudLogForReplay.extractReplay (sReplayFile, iStart, iEnd, fc.getSelectedFile ().getAbsolutePath ());
			}
		};
		
		taMarkLocation = new ThudSimpleAction ("Mark Current Location") {
			protected void doAction () {
				if (!bReplay)
					return;
				...
				iStart = myReplay.getCurrentLine();
				myReplay.setMarkedLine (iStart);
			}
		};
		
		taPauseReplay = new ThudSimpleAction ("Pause Replay") {
			protected void doAction () {
				if (!bReplay)
					return;
				...
				myReplay.setPaused (true);
			}
		};
		
		taContinueReplay = new ThudSimpleAction ("Continue Replay") {
			protected void doAction () {
				if (!bReplay)
					return;
				...
				myReplay.setPaused (false);
			}
		};
		
		taShowReplay = new ThudSimpleAction ("Show Replay") {
			protected void doAction () {
				//Create a file chooser
				JFileChooser fc = new JFileChooser();
				...
				sReplayFile = fc.getSelectedFile ().getAbsolutePath ();
				
				myLogger.Log("Show Replay :" + sReplayFile + ":");
				
				if (connected) // Clear our current connection first
					stopConnection();

				startReplay(sReplayFile);
		...
		taFastForward = new ThudSimpleAction ("FastForward Replay") {
			protected void doAction () {
				MUReplayConnection myConn = (MUReplayConnection)conn;
				
				myConn.fastForward ();
			}
		};
		
		taFastFastForward = new ThudSimpleAction ("FastFastForward Replay") {
			protected void doAction () {
				MUReplayConnection myConn = (MUReplayConnection)conn;
		...
		taResetDefaults = new ThudSimpleAction ("Reset to Default...") {
			protected void doAction () {
				File myThudConfig = new File (MUMapActions.configFilePath ());
		...
		taStopMapKeysAndClicks = new ThudSimpleAction ("Stop Map Mouse Clicks and Hot Keys") {
			protected void doAction () {
				doStopMapClicks();
			}
		};
		taStopMapKeysAndClicks.setSelected(prefs.bStopMapEvents);
	...
	private void bindMUMapAction (MUMapAction myAction)
	{
		if (!myAction.CanUseInChat ())
			return;
		
		myLogger.Log ("bindMUMapAction: 001 Binding :" + myAction.forConfigList () + ":");
		
		KeyStroke key;
		
		key = KeyStroke.getKeyStroke(myAction.iItem, myAction.iKeyModifiers, true);

		myLogger.Log ("bindMUMapAction: 002 Binding :" + key.toString () + ":");

		ThudMapAction myThudAction;
		
		myThudAction = new ThudMapAction (this, myAction);
		
		bindAction (key, myThudAction);
	}
	...
	private void bindAction (final KeyStroke key, final Action action) {
		...
		Object nameActionKey = action.getValue(Action.NAME);
		...
		default:
			...
			inputMap.put(key, null);
			inputMap.put(key, nameActionKey);
			inputMap = getRootPane().getInputMap (JComponent.WHEN_FOCUSED);
			inputMap.put(key, null);
			inputMap.put(key, nameActionKey);
			break;
		}
...
textPane.setRequestFocusEnabled(true);
	...
	textField.getActionMap().put("btthud.ENTER", new AbstractAction () {
		public void actionPerformed (final ActionEvent ae) {
			...
			myLogger.Log ("TypedCommand :" + typedText + ":");
	...
	textField.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0), "btthud.UP");
	textField.getActionMap().put("btthud.UP", new AbstractAction () {
		public void actionPerformed (final ActionEvent ae) {
	...
	textField.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0), "btthud.DOWN");
	textField.getActionMap().put("btthud.DOWN", new AbstractAction () {
		public void actionPerformed (final ActionEvent ae) {
    ...
    public void instantiateMapActions ()
    {
        if (mapActions != null)
        	return;
        
        // We need to tell if there is a config or not
        
        if (!MUMapActions.doesConfigExist ())
        {
        	ChooseMapOrChat myChoose = new ChooseMapOrChat ();
        	myChoose.BringMeUp ();
        }
    ...
    public void startReplay (String sReplayFName)
    {        
	    ...
	    if (prefs.bUse2Maps)
	    	tacMap2 = new MUTacticalMap (this, 1);
	    else
	    	tacMap2 = null;
	    ...
	    if (tacMap2 != null)
	    	data.addActionListener(tacMap2);
    ...
    {       
    	myLogger.Log("startConnection: 001 :" + host.toString () + ":");
    	...
    	myLogger.Log("startConnection: 002 :" + host.toString () + ":");

        bReplay = false;
        
        try
        {
        	myLogger.Log("startConnection: 003 :" + host.toString () + ":");

            // Setup some of the helper classes
            data = new MUData();

        	myLogger.Log("startConnection: 004 :" + host.toString () + ":");

            parse = new MUParse(textPaneWriter, data, prefs);
            parse.messageLine("*** Connecting... ***");
            
           	myLogger.Log("startConnection: 005 :" + host.toString () + ":");
	    ...
	    // Setup the rest of the helper classes.
	    tacMap = new MUTacticalMap (this, 0);
       	myLogger.Log("startConnection: 006.01 :" + host.toString () + ":");
	    ...
	    {
	      	myLogger.Log("startConnection: 006.02 :" + host.toString () + ":");

	    	tacMap2 = null;
	    }

       	myLogger.Log("startConnection: 007 :" + host.toString () + ":");

	    status = new MUStatus (this);
	    conList = new MUContactList (this);

	    myLogger.Log("startConnection: 008 :" + host.toString () + ":");

	    data.addActionListener(tacMap);
	    if (tacMap2 != null)
	    	data.addActionListener(tacMap2);
	    
       	myLogger.Log("startConnection: 009 :" + host.toString () + ":");

	    data.addActionListener(status);
	    data.addActionListener(conList);

       	myLogger.Log("startConnection: 010 :" + host.toString () + ":");
            ...
            parse.setCommands(commands);
           	...
           	myLogger.Log("startConnection: 011 :" + host.toString () + ":");

            // Let the text field get the keyboard focus
            setVisible(true);
            textField.grabFocus();

           	myLogger.Log("startConnection: 012 :" + host.toString () + ":");

            // Okay we're connected
            connected = true;

           	myLogger.Log("startConnection: 013 :" + host.toString () + ":");

            // Enable some menu stuff
            taStartStop.setEnabled(true);
           	myLogger.Log("startConnection: 014 :" + host.toString () + ":");

            taUpdateTacticalMapNow.setEnabled(true);
           	myLogger.Log("startConnection: 015 :" + host.toString () + ":");

            taDisconnect.setEnabled(true);
           	myLogger.Log("startConnection: 016 :" + host.toString () + ":");

            mapMenu.setEnabled(true);
           	myLogger.Log("startConnection: 017 :" + host.toString () + ":");
            ...
            windowMenu.setEnabled(true);
           	myLogger.Log("startConnection: 018 :" + host.toString () + ":");
        }
        catch (Exception e)
        {
            System.out.println("Error: " + e);
    ...
    public void updateTacMaps ()
    {
        if (tacMap != null)
            tacMap.newPreferences(prefs);
        if (tacMap2 != null)
        	tacMap2.newPreferences(prefs);
    }
    ...
    public void doStopMapClicks()
    {
        prefs.bStopMapEvents = !prefs.bStopMapEvents;
    }
    ...
    public void writePrefs()
    {
        ...
        if (tacMap2 != null)
        {
            prefs.tac2Loc = tacMap2.getLocation();
            prefs.tac2SizeX = tacMap2.getSize().width;
            prefs.tac2SizeY = tacMap2.getSize().height;
        }
    ...
    public MUConnectionBase getConn ()
    {
    	return conn;
    }
}
