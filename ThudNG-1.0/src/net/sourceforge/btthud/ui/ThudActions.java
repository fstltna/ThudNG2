...
//Action class to send a fixed command string.
class ThudMapAction extends AbstractAction {
	protected Thud thud;
	protected MUMapAction myAction;
	protected Logger myLogger;
	
	ThudMapAction (Thud thud, MUMapAction myAction)
	{
		super(myAction.sActions);

		myLogger = new Logger ("ThudMapAction");
	...
	public void actionPerformed (final ActionEvent ae) {
		myLogger.Log("actionPerformed: 001 " + myAction.forConfigList ());
		
		if (thud.conn == null)
			return;
		
		myLogger.Log("actionPerformed: 002 " + myAction.forConfigList ());
		
		try {
			myLogger.Log("actionPerformed: 003.01 " + myAction.forConfigList ());
			myAction.executeActions (thud, thud.getTacmap ().getMap (), (MouseEvent)null);
			myLogger.Log("actionPerformed: 003.02 " + myAction.forConfigList ());
		} catch (Exception e) {
			// TODO: Seems like it'd be more friendly to report
			// these errors in the main window, or in a modal
			// dialog.  Hiding things in the console is so like
